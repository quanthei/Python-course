...
#--------------------FUNCTIONS--------------------
def afficher_menu(nom_menu: str, menu_a_afficher: list):
    if len(menu_a_afficher) == 0: # On check si le menu n'est pas vide avant de l'afficher
        print(f"Le menu '{nom_menu.upper()}' ne contient aucun plat !")
    else: # On affiche le nom du menu + tous les plats qui le compose
        print(f"---- {nom_menu.upper()} ({len(menu_a_afficher)}) ----")
        for plat in range(len(menu_a_afficher)):
            print(f"{plat+1} - {menu_a_afficher[plat]}")
        # On affiche le premier et le dernier plat du menu
        print()
        print(f"-Premier plat du menu: {menu_a_afficher[0]}")
        print(f"-Dernier plat du menu: {menu_a_afficher[-1]}", end="\n")
